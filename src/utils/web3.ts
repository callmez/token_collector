...
export function setWalletProvider(newWallet: string = wallet) {
  ...
  if (wallet == "metamask") {
    ...
    // } else if (wallet == "ronin") {
    //   provider = (window as any).ronin;
  }
}
...
export async function getContract(address: string, abi: AbiItem[] | AbiItem) {
  const web3 = await getWeb3();
  return new web3.eth.Contract(abi, address) as Contract;
}
...
export async function contractCall({
  ...
  let gasAmount = 80000;
  try {
    gasAmount = await contract.methods[method]
      .apply(null, params)
      .estimateGas({ from });
  } catch (e) {
